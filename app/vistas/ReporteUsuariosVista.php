...
function Header()
{
    ...
    $this->Cell(120,10,'REPORTE DE USUARIOS ADMINISTRATIVOS',1,0,'C',1);
    ...
    $this->Cell(30,10,utf8_decode('STATUS'),1,1,'C',1);
...
function Footer()
{
  ...
  ob_start();
  //'L','mm','A4' L para horizontal y el tamaño
  $pdf = new PDF();
  $pdf -> AliasNbPages();
  $pdf->AddPage();
  $w=50;
  ...
  for($i=0; $i<count($datos['data']); $i++){
    ...
    $x=$pdf->GetX();
    $pdf->MyCell(50,10,$x,' '.utf8_decode($datos["data"][$i]["nombre"]) ,1,0,'C',0);
    $pdf->Cell(60,10,utf8_decode($datos["data"][$i]["correo"]) ,1,0,'C',0);
    if ($datos["data"][$i]["status"]==1) {
     // $pdf->Cell(30,10,$datos["data"][$i]["status"],1,1,'C',0);
      $pdf->Cell(30,10,'Activo',1,1,'C',0);;
    }elseif ($datos["data"][$i]["status"]==0) {
      $pdf->Cell(30,10,'Inactivo',1,1,'C',0);;
    }
   }
  $pdf->Output();
  ob_end_flush();
